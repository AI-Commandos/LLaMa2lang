...
def format_dpo(thread, system_instruction, bad_child):
    ...
    for i in range(0, len(thread) - 2, 2):
        user_msg = thread[i]['text'] if i < len(thread) else ""
        model_answer = thread[i+1]['text'] if i+1 < len(thread) else ""
        formatted_thread['prompt'] += f"{user_msg} [/INST] {model_answer} </s>"
        if i+2 < len(thread):
...
def find_children_and_highest_ranked_child(df, parent_id):
    children = df[df['parent_id'] == parent_id]
    max = children['rank'].max()

    if not children.empty:
      return children[children['rank'] == max], children[children['rank'] != max]
    return None, None
    ...
    for fold in folds:
        ...
        with tqdm(total=len(root_messages)) as pbar:
            for _, root_message in root_messages.iterrows():
                ...
                good_child, bad_children = find_children_and_highest_ranked_child(df, root_message[base_dataset_id_field])

                while good_child is not None and not good_child.empty:
                    thread.append({'text': good_child.iloc[0][base_dataset_text_field]})

                    for bad_child in bad_children.iterrows():
                        threads[fold].append(format_dpo(thread, instruction_prompt, bad_child))

                    good_child, bad_children = find_children_and_highest_ranked_child(df, good_child[base_dataset_id_field].iloc[0])
